...
* **Quy tắc cốt lõi (Cách tạo ra & Xác định Condition):**
  * **Giới hạn phạm vi áp dụng:** KHÔNG dùng cho toàn hệ thống hay kiểm tra định dạng chữ/số đơn lẻ (đó là việc của EP/BVA). Chỉ áp dụng cho hệ thống có **logic tổ hợp (Combinatorial Logic)** dẫn đến nhiều ngã rẽ hành vi.
  * **Nguyên tắc 1: Tách biệt rõ ràng Sự kiện (Trigger) và Điều kiện (Condition):**
    * *Sự kiện (Trigger):* Thao tác bắt đầu quy trình (VD: Bấm nút "Tạo tài khoản", Bấm "Đăng nhập"). Đây là Tiền đề (Pre-requisite) để chạy bảng, KHÔNG ĐƯỢC đưa vào làm dòng Condition trong bảng.
    * *Điều kiện (Condition):* Trạng thái của dữ liệu hoặc hệ thống tại thời điểm kiểm tra (mang giá trị `T`/`F`).
  * **Nguyên tắc 2: Xác định theo Chốt chặn logic (Validation Gates):**
    * Đặt câu hỏi: *"Hệ thống cần những dữ liệu/trạng thái nghiệp vụ nào để quyết định cho qua hoặc dừng lại?"*
    * *Chốt 1:* Đã điền đủ các trường bắt buộc chưa? (Tính đầy đủ)
    * *Chốt 2:* Các trường dữ liệu có hợp lệ hết không? (Tính hợp thức)
    * *Chốt 3:* Mật khẩu xác nhận có khớp mật khẩu gốc không? (Ràng buộc liên trường)
    * *Chốt 4:* Email/Tài khoản có bị trùng trên hệ thống không? (Tính duy nhất)
    * Các trường dữ liệu độc lập không gộp chung vào 1 Condition duy nhất làm mất khả năng cô lập lỗi.
  * **Nguyên tắc 3: Tối giản hóa và Thu gọn luật (Rule Reduction):**
    * Khi một Condition mang giá trị `F` làm gián đoạn toàn bộ luồng xử lý (VD: Bỏ trống form thì báo lỗi ngay), các Condition phía sau không còn ý nghĩa. Đánh dấu gạch ngang `-` (*Don't care*) để gộp nhánh và giảm tối đa các luật không khả thi trên thực tế.
